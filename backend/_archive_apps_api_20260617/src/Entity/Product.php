...
class Product
{
    ...
    #[ORM\OneToMany(mappedBy: 'product', targetEntity: LocalProduct::class, cascade: ['remove'])]
    ...
    public function setPrice(string|float $price): self { $this->price = number_format((float) $price, 2, '.', ''); return $this; }
    ...
    public function setPrepTime(?int $prepTime): self { $this->prepTime = $prepTime; return $this; }
    ...
    public function setAllergens(iterable $allergens): self { $this->allergens->clear(); foreach ($allergens as $allergen) { $this->allergens->add($allergen); } return $this; }
}
