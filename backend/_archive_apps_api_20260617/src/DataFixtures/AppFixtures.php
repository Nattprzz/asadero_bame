...
final class AppFixtures extends Fixture
{
    ...
    private function loadUsers(ObjectManager $manager): array
    {
        ...
        foreach ([
            ...
            $user = (new User())->setUsername($username)->setName($name)->setSurname($surname)->setEmail($email)->setRoles($roles);
    ...
    private function loadCatalogBase(ObjectManager $manager): array
    {
        ...
        foreach ([
            ...
            $category = (new Category())->setSlug($slug)->setName($name)->setDescription($description)->setSortOrder($sortOrder);
        ...
        foreach ([
            ...
            $allergen = (new Allergen())->setSlug($slug)->setName($name);
    ...
    private function loadProducts(ObjectManager $manager, array $categories, array $allergens): array
    {
        ...
        foreach ($rows as [$slug, $name, $description, $categorySlug, $price, $featured, $weight, $prepTime, $allergenSlugs]) {
            ...
            $product->setAllergens(array_map(fn (string $a) => $allergens[$a], $allergenSlugs));
    ...
    private function loadLocalsAndStock(ObjectManager $manager, array $products): Local
    {
        ...
        foreach ([
            ...
            $manager->persist((new LocalProduct())->setLocal($local)->setProduct($products[$slug])->setStock($stock));
        }
